...
//Antai <AI_BSP_FACTORY> <luobowen> <2021-09-09> add for factory begin
enum rtc_spare_enum {
	RTC_FGSOC = 0,
	RTC_ANDROID,
	RTC_FAC_RESET,
...
//Antai <AI_BSP_FACTORY> <luobowen> <2021-09-09> add for factory end
/*
 * RTC_PDN1:
...
void rtc_mark_bootmenu(void)
{
	...
	pr_notice("%s\n", __func__);
	spin_lock_irqsave(&rtc_misc->lock, flags);
	mtk_rtc_set_spare_register(RTC_BOOT_MENU, 0x1);
	spin_unlock_irqrestore(&rtc_misc->lock, flags);
}
...
void rtc_mark_factory(void)
{
	...
	pr_notice("%s\n", __func__);
	spin_lock_irqsave(&rtc_misc->lock, flags);
	mtk_rtc_set_spare_register(RTC_FACTORY, 0x1);
	spin_unlock_irqrestore(&rtc_misc->lock, flags);
}
#endif
//Antai <AI_BSP_FACTORY> <luobowen> <2021-09-09> add for factory end
