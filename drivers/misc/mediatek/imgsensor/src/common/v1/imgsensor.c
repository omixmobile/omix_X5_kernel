...
//Antaiui <AI_BSP_CAMERA> <hehl> <2020-03-16> add for camera display begin
...
#endif
//Antaiui <AI_BSP_CAMERA> <hehl> <2020-03-16> add for camera display end
#ifdef CONFIG_MTK_SMI_EXT
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	//Antaiui <AI_BSP_CAMERA> <hehl> <2020-03-16> add for camera display begin
	#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
	struct ai_device_info* cam_device_info;
	//CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx = psensor->inst.sensor_idx;
	cam_device_info = kmalloc(sizeof(struct ai_device_info), GFP_KERNEL);
	#endif
	//Antaiui <AI_BSP_CAMERA> <hehl> <2020-03-16> add for camera display end
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		{
			// PK_DBG(" main Sensor found SensorIdx = %d,sensor_name=%s\n", psensor->inst.sensor_idx,psensor_inst->psensor_name);
			cam_device_info->ai_dev_type = AI_DEVICE_TYPE_MAIN_CAM;
			strcpy(cam_device_info->name, psensor_inst->psensor_name);
			ai_set_device_info(*cam_device_info);
		}
		if (IMGSENSOR_SENSOR_IDX_MAIN2 == psensor->inst.sensor_idx)
		{
			// PK_DBG(" main2 Sensor found SensorIdx = %d,sensor_name=%s\n", psensor->inst.sensor_idx,psensor_inst->psensor_name);
			cam_device_info->ai_dev_type = AI_DEVICE_TYPE_MAIN2_CAM;
			strcpy(cam_device_info->name, psensor_inst->psensor_name);
			ai_set_device_info(*cam_device_info);
		}
		if (IMGSENSOR_SENSOR_IDX_SUB == psensor->inst.sensor_idx)
		{
			// PK_DBG(" sub Sensor found SensorIdx = %d,sensor_name=%s\n", psensor->inst.sensor_idx,psensor_inst->psensor_name);
			cam_device_info->ai_dev_type = AI_DEVICE_TYPE_SUB_CAM;
			strcpy(cam_device_info->name, psensor_inst->psensor_name);
			ai_set_device_info(*cam_device_info);
		}
		if (IMGSENSOR_SENSOR_IDX_SUB2 == psensor->inst.sensor_idx)
		{
			// PK_DBG(" sub2 Sensor found SensorIdx = %d,sensor_name=%s\n", psensor->inst.sensor_idx,psensor_inst->psensor_name);
			cam_device_info->ai_dev_type = AI_DEVICE_TYPE_SUB2_CAM;
			strcpy(cam_device_info->name, psensor_inst->psensor_name);
			ai_set_device_info(*cam_device_info);
		}
		if (IMGSENSOR_SENSOR_IDX_MAIN3 == psensor->inst.sensor_idx)
		{
			// PK_DBG(" main3 Sensor found SensorIdx = %d,sensor_name=%s\n", psensor->inst.sensor_idx,psensor_inst->psensor_name);
			cam_device_info->ai_dev_type = AI_DEVICE_TYPE_MAIN3_CAM; //not 1901 prj other modify for sub2
			strcpy(cam_device_info->name, psensor_inst->psensor_name);
			ai_set_device_info(*cam_device_info);
		}
		#endif
