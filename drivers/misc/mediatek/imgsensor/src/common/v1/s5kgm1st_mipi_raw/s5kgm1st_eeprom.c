...
#define CAM_CAL_DTNAME "mediatek,camera_main_eeprom" //CAM_CAL_DRV0
#endif
#ifndef CAM_CAL_DTNAME_I2C
#define CAM_CAL_DTNAME_I2C "mediatek,cam_cal_s5kgm1st"
#endif

#define CAM_CAL_DRVNAME "CAM_CAL_DRV1" //CAM_CAL_DRV0
#define CAM_CAL_CLASSNAME "CAM_CAL1"   //CAM_CAL0

#define EEPROM_READ_ID 0XA0	 //0xA8
#define EEPROM_WRITE_ID 0XA1 //0xA9
...
static void preload_eeprom_data(void)
{
	...
	{
		CAM_CALINF("s5kgm1st eeprom start!\n");
...
int eeprom_read_region(u32 addr, u8 *data, u32 size)
{
	unsigned short curAddr = (unsigned short)addr;
	u8 *buff = data;
...
static void CAM_CAL12_work_func(struct work_struct *data)
{
	...
	{
		retry--;
		CAM_CALERR("Error : s5kgm1st read eeprom fail\n");
		if (CAM_CAL2_wq1 != NULL)
...
#define GN_BSP_CAM_CAL_S5KGM1ST
#ifdef GN_BSP_CAM_CAL_S5KGM1ST
...
#if 1
	{
		...
		for (i = 0; i < EEPROM_DATA_SIZE; i++)
		{
			/*if(i == 0x0020)
				...
				printk("\n\taddr[0x%04x]:", i);*/
			if (i >= 0x001E && i <= 0x0030)
			{
				printk("LSC info 0x%02x : %02x \n", i, eeprom_Data[i]);
			}
		}
		printk("\n=====================OTP end cam_cal_s5kgm1st=========================\n\n");
	}
#endif
