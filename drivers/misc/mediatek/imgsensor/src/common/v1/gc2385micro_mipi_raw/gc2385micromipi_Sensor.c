...
#include "gc2385micromipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC2385MICRO_camera_sensor"
#define LOG_1 LOG_INF("GC2385MICRO,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC2385MICRODuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC2385MICRO_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC2385MICROMIPI analogic gain 1x, GC2385MICROMIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 1.43x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 2x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 2.84x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 3.97x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_6<= iReg)&&(iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 5.68x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	else if((ANALOG_GAIN_7<= iReg)&&(iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 8.14x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	...
	else if((ANALOG_GAIN_8<= iReg)&&(iReg < ANALOG_GAIN_9))
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 11.34x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	...
	else
	{
		...
		write_cmos_sensor(0xb1,temp>>8);
		write_cmos_sensor(0xb2,temp&0xff);
		LOG_INF("GC2385MICROMIPI analogic gain 16.23x , GC2385MICROMIPI add pregain = %d\n",temp);
	}
	return gain;
...
//xuel compatible <
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC2385MICRODuringTestPattern = KAL_FALSE;
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	imgsensor.shutter = shutter;
	spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
	LOG_INF("[park] shutter =%d, framelength =%d", shutter,frame_length);
	LOG_INF("[park] min_frame_length =%d, max_frame_length =%d, min_shutter=%d", imgsensor.min_frame_length,
		imgsensor_info.max_frame_length, imgsensor_info.min_shutter);

	spin_lock(&imgsensor_drv_lock);

	LOG_INF("set_shutter_frame_length Exit! shutter =%d, framelength =%d\n unique", shutter,imgsensor.frame_length*2);
...
UINT32 GC2385MICROMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC2385MICROMIPI_RAW_SensorInit    */
