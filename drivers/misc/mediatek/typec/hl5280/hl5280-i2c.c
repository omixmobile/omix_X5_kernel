...
static int fsa4480_usbc_analog_setup_switches(struct fsa4480_priv *fsa_priv)
{
	...
	if (fsa_priv->plug_state) {
		...
		if ( 0 /*gpio_is_valid(fsa_priv->hs_det_pin)*/) {
...
//            }
		...
		if (0 /*gpio_is_valid(fsa_priv->hs_det_pin)*/) {
            ...
            if (0/*(20630 == get_project()) || (20631 == get_project()) || (20632 == get_project())*/) {
                pinctrl1 = devm_pinctrl_get(dev);
                if (IS_ERR(pinctrl1)) {
                    ret = PTR_ERR(pinctrl1);
                    pr_err("can not find fsa4480 pinctrl 0\n");
...
struct fsa4480_priv *fsa_priv_global;
void fsa4480_reset(void)
{
    regmap_write(fsa_priv_global->regmap, FSA4480_RESET, 0x01);
    printk("%s\n", __func__);
}
...
//Antai <AI_BSP_Audio> <hehl> <2022-06-13> coompatiable ic YHM1003 driver begin
	...
	if (HL5280_DEVICE_REG_VALUE == reg_value) {
		...
		dev_err(fsa_priv->dev, "%s: switch chip fail\n", __func__);
