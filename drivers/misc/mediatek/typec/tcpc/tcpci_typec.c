...
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 begin
static int cc1_num = -1;
static int cc2_num = -1;
static int typec_direction_state=0;
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 end
...
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 begin
	cc1_num = typec_get_cc1();
	cc2_num = typec_get_cc2();
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 end
...
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 begin
extern int typec_otg_state;
static ssize_t ai_typec_direction_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	TYPEC_INFO("%s cc1_num=%d,cc2_num=%d\n",__func__,cc1_num,cc2_num);
	if(1 == typec_otg_state)
	{
		TYPEC_INFO("otg insert\n");
		typec_direction_state = 0;
		return sprintf(buf, "%d", typec_direction_state);
	}
	if(cc1_num != cc2_num && (cc1_num | cc2_num) )
	{
		if(0 == cc1_num)
		{
			TYPEC_INFO("typec opposite direction insert\n");
			typec_direction_state = 2;
		}
		else if(0 == cc2_num)
		{
			TYPEC_INFO("typec positive direction insert\n");
			typec_direction_state = 1;
		}		
	}else if(cc1_num == cc2_num)
	{
		typec_direction_state=0;
		TYPEC_INFO("typec no insert\n");
	}	
	return sprintf(buf, "%d", typec_direction_state);
}
static DEVICE_ATTR (typec_direction, S_IRUGO|S_IWUSR, ai_typec_direction_show, NULL);

static struct device_attribute *typec_direction_attr_list[] = {
    &dev_attr_typec_direction,
};
static struct platform_device ai_typec_direction_device = {
    .name   = "typec_direction_state",
    .id     = -1,
};
static int tpd_create_attr(struct device *dev)
{
    int idx, err = 0;
    int num = (int)(sizeof(typec_direction_attr_list)/sizeof(typec_direction_attr_list[0]));
    ...
    {
        if(err != device_create_file(dev, typec_direction_attr_list[idx]))
        {
            TYPEC_INFO("driver_create_file failed.\n");
            break;
        }
    }
...
static int typec_direction_delete_attr(struct device *dev)
{
    int idx ,err = 0;
    int num = (int)(sizeof(typec_direction_attr_list)/sizeof(typec_direction_attr_list[0]));
    if (dev == NULL)
    ...
    for(idx = 0; idx < num; idx++)
    {
        device_remove_file(dev,typec_direction_attr_list[idx]);
    }
    return err;
}
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 end
...
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 begin
	ret = platform_device_register(&ai_typec_direction_device);
    if (ret)
    {
        TYPEC_INFO("create ai_typec_direction_device failed\n");
    }
    ret = tpd_create_attr(&(ai_typec_direction_device.dev));
    if(ret)
    {
        TYPEC_INFO("create ai_typec_direction_device attr failed \n");
    }
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 end
...
{
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 begin
	typec_direction_delete_attr(&(ai_typec_direction_device.dev));
	platform_device_unregister(&ai_typec_direction_device);
//Antaiui <AI_BSP_USB> <hehl> <2021-08-10> add typec direction interface for 2003 end	
}
