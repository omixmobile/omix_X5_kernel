...
#endif
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-17> add charger mmi start
#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
{
	...
	strcpy(ai_chg_hw_info.name, "PMIC6370");
	ai_set_device_info(ai_chg_hw_info);
}
#endif
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-17> add charger mmi end
	return ret;
}
...
//Antai <AI_BSP_CHG> <yaoyc> <2022-11-24> modify for framework get vbus begin
		if (chg_data->psy_desc.type == POWER_SUPPLY_TYPE_USB)
			val->intval = 500000;
		else {
			mt6370_get_adc(chg_data, MT6370_ADC_IBUS, &val->intval);
		}
		dev_info(chg_data->dev, "%s: current_max = %d\n", __func__, val->intval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (chg_data->psy_desc.type == POWER_SUPPLY_TYPE_USB)
			val->intval = 5000000;
		else {	
			mt6370_get_adc(chg_data, MT6370_ADC_VBUS_DIV5, &val->intval);
		}
		dev_info(chg_data->dev, "%s: voltage_max = %d\n", __func__, val->intval);
//Antai <AI_BSP_CHG> <yaoyc> <2022-11-24> modify for framework get vbus end
		break;
	default:
		ret = -ENODATA;
		break;
