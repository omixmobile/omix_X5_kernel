...
#include "hwmsensor.h"
//Antaiui <AI_BSP_SNS> <hehl> <2021-12-02> add for sensor display start
...
#endif
//Antaiui <AI_BSP_SNS> <hehl> <2021-12-02> add for sensor display end
...
//Antaiui <AI_BSP_SNS> <hehl> <2021-12-02> add for sensor display start
#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
	struct ai_device_info ai_sen_hw_info;
#endif
//Antaiui <AI_BSP_SNS> <hehl> <2021-12-02> add for sensor display end
	for (handle = accel; handle < maxhandle; ++handle) {
...
//Antaiui <AI_BSP_SNS> <hehl> <2021-12-02> add for sensor display start
#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK		
		pr_err("sensor(%d) register sensor_name %s\n", sensor,devinfo.name);
		switch(sensor){
			case ID_ACCELEROMETER:
				ai_sen_hw_info.ai_dev_type = AI_DEVICE_TYPE_ACCELEROMETER;
				strcpy(ai_sen_hw_info.name, devinfo.name);
				break;
			case ID_GYROSCOPE:
				ai_sen_hw_info.ai_dev_type = AI_DEVICE_TYPE_GYROSCOPE;
				strcpy(ai_sen_hw_info.name, devinfo.name);
				break;
			case ID_MAGNETIC:
				ai_sen_hw_info.ai_dev_type = AI_DEVICE_TYPE_MAGNETIC_FIELD;
				strcpy(ai_sen_hw_info.name, devinfo.name);
				break;
			case ID_LIGHT:
				ai_sen_hw_info.ai_dev_type = AI_DEVICE_TYPE_LIGHT;
				strcpy(ai_sen_hw_info.name, devinfo.name);
				break;
			case ID_PROXIMITY:
				ai_sen_hw_info.ai_dev_type = AI_DEVICE_TYPE_PROXIMITY;
				strcpy(ai_sen_hw_info.name, devinfo.name);
				break;
			default:
				pr_err("sensor type not aviables\n");
				break;								
		}
		ai_set_device_info(ai_sen_hw_info);
#endif		
//Antaiui <AI_BSP_SNS> <hehl> <2021-12-02> add for sensor display end
