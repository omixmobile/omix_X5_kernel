...
typedef enum {
    ...
    FF_PINCTRL_STATE_RST_ACT,
    FF_PINCTRL_STATE_INT_ACT,
	FF_PINCTRL_STATE_SPI_CS_GPIO,
    FF_PINCTRL_STATE_SPI_CK_GPIO,
    FF_PINCTRL_STATE_SPI_MOSI_GPIO,
    FF_PINCTRL_STATE_SPI_MISO_GPIO,
    FF_PINCTRL_STATE_MAXIMUM /* Array size */
} ff_pinctrl_state_t;
#endif
...
#else
static const char *g_pinctrl_state_names[FF_PINCTRL_STATE_MAXIMUM] = {
    "fpsensor_finger_power_high","fpsensor_finger_power_low","fpsensor_finger_rst_low","fpsensor_finger_rst_high","fpsensor_eint_as_int","fpsensor_cs_to_gpio","fpsensor_ck_to_gpio","fpsensor_mo_to_gpio","fpsensor_mi_to_gpio"
};
#endif
...
int ff_ctl_spi_to_gpio(void)
{
	int err = 0;
    printk("'%s' enter.", __func__);
	
	for (int i = FF_PINCTRL_STATE_INT_ACT + 1; i < FF_PINCTRL_STATE_MAXIMUM; ++i) {
		err = pinctrl_select_state(g_context->pinctrl, g_context->pin_states[i]);
	...
	if (err) {
		printk("%s() pinctrl_select_state(FF_PINCTRL_STATE_RST_CLR) failed.\n", __FUNCTION__);
	}
