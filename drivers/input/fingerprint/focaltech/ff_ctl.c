...
#if CONFIG_MICROTRUST_TEE_SUPPORT
/**
...
//04190000 0000 0000 0000 00 00 00 00 00 00.ta
static struct TEEC_UUID focaltech_vendor_uuid = { 0x04190000, 0x0000, 0x0000,
...
//Antaiui <AI_BSP_Fingerprint> <yaoyc> <2022-04-24> add for 41486 fixed idle current test fail on ATA test begin  
struct tag_bootmode {
  	unsigned int size;
  	unsigned int tag;
...
static int ff_get_boot_mode(void)
{
    struct device_node *np_chosen;
  	...
  	if (!tag) {
  		...
  		pr_err("bootmode: 0x%x boottype: 0x%x\n",
  				tag->bootmode, tag->boottype);
		return tag->bootmode;
	}
	return NORMAL_BOOT;
}
//Antaiui <AI_BSP_Fingerprint> <yaoyc> <2022-04-24> add for 41486 fixed idle current test fail on ATA test end
////////////////////////////////////////////////////////////////////////////////
...
extern int ff_ctl_spi_to_gpio(void);
...
static int ff_ctl_report_key_event(struct input_dev *input, ff_key_event_t *kevent)
{
    ...
    //Antaiui <AI_BSP_FP> <zhul> <2022-09-24> add for 44921 report camera key up value after reporting down value begin
    if((kevent->code == 0x01f7)&&(kevent->value == 0x01))
    {
        int i = 20000;
        while (i--);
        input_report_key(input, kevent->code, 0x00);
        input_sync(input);
    }
    //Antaiui <AI_BSP_FP> <zhul> <2022-09-24> add for 44921 report camera key up value after reporting down value end
...
static int ff_ctl_register_input(void)
{
	...
	input_set_capability(g_context->input, EV_KEY, 0x1f7);
...
static int __init ff_ctl_driver_init(void)
{
    int err = 0;
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
	char fpc_id[3][20]={"focaltech-ft9362","null","null"};
...
#endif
#if CONFIG_MICROTRUST_TEE_SUPPORT
	FF_LOGI("FocalTech fingerprint TA UUID.");
	memcpy(&uuid_fp , &focaltech_vendor_uuid , sizeof(struct TEEC_UUID));
#endif
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
...
//Antaiui <AI_BSP_Fingerprint> <yaoyc> <2022-04-24> add for 41486 fixed idle current test fail on ATA test begin 
#if 1
	if (ff_get_boot_mode() == FACTORY_BOOT) {
//Antaiui <AI_BSP_Fingerprint> <yaoyc> <2022-04-24> add for 41486 fixed idle current test fail on ATA test end
		FF_LOGI("FocalTech fingerprint device need release ff_wake_lock in  FACTORY_BOOT.");

		ff_ctl_enable_power(0);
		ff_ctl_enable_spiclk(0);
