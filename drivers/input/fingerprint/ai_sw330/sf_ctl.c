...
//Antaiui <AI_BSP_FINGER> <hehl> <2022-02-18> add finger mmi info begin
#include <linux/of_gpio.h> 
#include <linux/gpio.h>
#include <linux/of_device.h>
#include <linux/of_address.h>
static int fp_vendor_gpio=0; 
static int ai_fp_get_vendor_id(void);
...
#endif
//Antaiui <AI_BSP_FINGER> <hehl> <2022-02-18> add finger mmi info end
//---------------------------------------------------------------------------------
...
static int ai_fp_get_vendor_id(void)
{
...
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
	char fpc_id[3][20]={"TRULY_SW9058","SW330","null"};
	struct ai_device_info ai_fpc_hw_info;
#endif
	
	dev_node = of_find_compatible_node(NULL, NULL, "mediatek,antaiui_fp");
    if (!dev_node) {
    ...
    fp_vendor_gpio = of_get_named_gpio(pdev->dev.of_node,"ai_fp_vendor_gpio",0);
    
	//pinctrl_select_state(goodix_pinctrl,gf518m_vendor_pu);
	//pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_vendor_pu);
	//pinctrl_select_state(sf_pinctrl, sf_pin_states[STATE_VENDOR_SET]);
	

	ret = gpio_request(fp_vendor_gpio,"cy_fp");
			
	if(ret<0)
	{
		pr_err("cy_fp_vendor_gpio gpio_request failed ! /n ");
	}
			
	vendor_id = gpio_get_value(fp_vendor_gpio);
			
	printk(" gf_get_vendor_id func get vendor_id = %d ", vendor_id);
			
	gpio_free(fp_vendor_gpio);
			

      // pinctrl_select_state(gf_dev->pinctrl_gpios,gf_dev->pins_vendor_pd); 
    
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
	ai_fpc_hw_info.ai_dev_type = AI_DEVICE_TYPE_FINGER;
	if(1 == vendor_id)
	{	
			snprintf(ai_fpc_hw_info.name, AI_DEVICE_NAME_LEN, "%s",fpc_id[0]);
	}else {
			snprintf(ai_fpc_hw_info.name, AI_DEVICE_NAME_LEN, "%s",fpc_id[1]);
	}
	ai_set_device_info(ai_fpc_hw_info);
#endif 
...
static irqreturn_t sf_ctl_device_irq(int irq, void *dev_id)
{
   ...
   //__pm_stay_awake(sf_ctl_dev.wakelock);
#else
...
static int sf_ctl_report_key_event(struct input_dev *input, sf_key_event_t *kevent)
{
	...
	//Antai <AI_BSP_FING> <wutj> <2022-11-24> add a shoot picture function start
    if(kevent->value==1){
    input_report_key(input, 0x1f7 , 1);
    input_sync(input);
    input_report_key(input, 0x1f7 , 0);
    input_sync(input);
    }
	//Antai <AI_BSP_FING> <wutj> <2022-11-24> add a shoot picture function end
...
static int sf_probe(sf_device_t *dev)
{
	...
	sf_ctl_dev.wakelock = wakeup_source_register(NULL,"ili_wakelock");
#else
...
static int sf_ctl_init_input(void)
{
	...
	//Antai <AI_BSP_FING> <wutj> <2022-11-24> add a shoot picture function start
    input_set_capability(sf_ctl_dev.input, EV_KEY, 0x1f7);
	//Antai <AI_BSP_FING> <wutj> <2022-11-24> add a shoot picture function end
