...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
	...
	AWLOGD(aw9610x->dev, "channel2 = 0x%d", curr_status_val);
	if (aw9610x->channels_arr == NULL) {
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		...
		//Antai <AI_BSP_SENSOR> <wutj> <2022-08-01>if intterupt is  trigger,this is repaot to framework start
		if(aw9610x->channels_arr[i].last_channel_info!=curr_status)
		{
			sar_report(aw9610x->sar_num,i,curr_status);
		}
		//Antai <AI_BSP_SENSOR> <wutj> <2022-08-01>if intterupt is  trigger,this is repaot to framework end
