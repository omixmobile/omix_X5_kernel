...
#if WAKEUP_GESTURE
extern int ovt_wake_switch;
extern int ovt_gesture_switch;
#endif
...
//Antaiui <AI_BSP_TP> <chenht> <2022-9-20> add for tp information begin
...
int ovt_register_hardware_info(void)
{
    int ret = 0;
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
    struct ai_device_info ai_tp_hw_info;
    ai_tp_hw_info.ai_dev_type = AI_DEVICE_TYPE_TP;
    snprintf(ai_tp_hw_info.name, AI_DEVICE_NAME_LEN, "ai_omnivision_tcm[%d]",tcm_hcd->packrat_number);
    ai_set_device_info(ai_tp_hw_info);
    return ret;
}
#endif
//Antaiui <AI_BSP_TP> <chenht> <2022-9-20> add for tp information end
...
static int ovt_tcm_identify(struct ovt_tcm_hcd *tcm_hcd, bool id)
{
	...
	//Antaiui <AI_BSP_TP> <chenht> <2022-9-20> add for tp information begin
	#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
	if (tcm_hcd->packrat_number > 0 ){
		ovt_register_hardware_info();
	}
	#endif
	//Antaiui <AI_BSP_TP> <chenht> <2022-9-20> add for tp information end
...
//Antai <AI_BSP_TP> <chenht> <2022-12-21> disable avdd and avee for 2206 begin
extern int PMU_db_pos_neg_disable_delay(int ms);
//Antai <AI_BSP_TP> <chenht> <2022-12-21> disable avdd and avee for 2206 end
//Antai <AI_BSP_TP> <chenht> <2022-12-21> disable avdd and avee for 2206 begin
int is_gesture_mode(void) {
...
#if WAKEUP_GESTURE
	if(ovt_gesture_switch || ovt_wake_switch)
		ret = 1;
	else
		ret = 0;
#else
	ret = 0;
#endif
	LOGI(tcm_hcd->pdev->dev.parent,"is_gesture_mode ret =%d \n",ret);
	return ret;
}
...
//Antai <AI_BSP_TP> <chenht> <2022-12-21> add for 2206 gesture end
	...
	LOGI(tcm_hcd->pdev->dev.parent,"in_suspend = %d",tcm_hcd->in_suspend);
	if (tcm_hcd->in_suspend)
		return 0;
	...
	LOGI(tcm_hcd->pdev->dev.parent,"touch_suspend = %d",tcm_hcd->wakeup_gesture_enabled);
...
//Antai <AI_BSP_TP> <chenht> <2022-12-21> disable avdd and avee for 2206 begin
	...
	if (ovt_gesture_switch || ovt_wake_switch){
		...
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
					"ovt_tcm_suspend:Failed to exit deep sleep\n");
			return retval;
		}
		pr_info("Enter into gesture(suspend) , enable irq");
	} else {
...
//Antai <AI_BSP_TP> <chenht> <2022-12-21> add for 2206 gesture begin
	retval = ai_ovt_tpd_feature_init_data();
	if (retval < 0) {
		//LOGE(tcm_hcd->pdev->dev.parent,
			//"Failed to initialze ai_ovt_tpd_feature_init_data\n");
		pr_info("Failed to initialze ai_ovt_tpd_feature_init_data\n");
	} else {
		//LOGE(tcm_hcd->pdev->dev.parent,
			//"Successed to initialze ai_ovt_tpd_feature_init_data\n");
		pr_info("Successed to initialze ai_ovt_tpd_feature_init_data\n");
	}
//Antai <AI_BSP_TP> <chenht> <2022-12-21> add for 2206 gesture end
...
//Antaiui <AI_BSP_CTP> <chenht> <2022-9-1> add tp test  begin
ssize_t ovt_test_show(
...
//Antaiui <AI_BSP_CTP> <chenht> <2022-9-1> add tp test  end
...
//Antai <AI_BSP_TP> <yaoyc> <2022-09-26> modify for 45003 to fixed sometimes tp not work after power on begin
module_init(ovt_tcm_module_init);
//Antai <AI_BSP_TP> <yaoyc> <2022-09-26> modify for 45003 to fixed sometimes tp not work after power on end
