...
//Antaiui <AI_BSP_TP> <chenht> <2023-02-06> add for mmi tp information begin
#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
#include  <linux/ai_device_check.h>
#endif
//Antaiui <AI_BSP_TP> <chenht> <2023-02-06> add for mmi tp information end
...
int chipone_register_hardware_info(struct cts_device *cts_dev)//struct cts_device *cts_dev
{
    int ret = 0;
    struct ai_device_info ai_tp_hw_info;
    cts_info("CHT_2023 MTK device check");
    ...
    snprintf(ai_tp_hw_info.name, AI_DEVICE_NAME_LEN, "chipone_icnl9916_FW[0x%04x]",cts_dev->fwdata.version);
...
void cts_firmware_upgrade_work(struct work_struct *work)
{
    ...
    //Antaiui <AI_BSP_TP> <chenht> <2023-02-06> add for mmi tp information begin
    #ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
    chipone_register_hardware_info(cts_dev);
    #endif
    //Antaiui <AI_BSP_TP> <chenht> <2023-02-06> add for mmi tp information end
    if (ret == 0)
