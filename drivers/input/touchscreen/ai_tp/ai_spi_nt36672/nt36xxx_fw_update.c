...
static int32_t nvt_bin_header_parser(const u8 *fwdata, size_t fwsize)
{
	 ...
	 */
	ts->wkg_info = (fwdata[0x28] & 0x20) >> 5;
	...
	for (list = 0; list < partition; list++) {
		...
		if (list >= (ilm_dlm_num + info_sec_num)) {
			...
			if ((list == (ilm_dlm_num + info_sec_num)) && (ts->wkg_info)) {
...
//[20190426,jx]Disable it 
//	Following code is only for NT36525
...
static int32_t nvt_check_gesture_checksum(uint32_t SRAM_addr, uint32_t size, uint32_t crc)
{
	...
	if (nvt_read_max >= sizeof(ts->rbuf)) {
		NVT_ERR("read len %d is larger than buffer size %ld\n",
				nvt_read_max, sizeof(ts->rbuf));
		return -EINVAL;
	} else if (nvt_read_max >= NVT_TRANSFER_LEN) {
		NVT_ERR("read len %d is larger than NVT_TRANSFER_LEN\n", nvt_read_max);
		return -EINVAL;
	}
	...
	for (i = 0 ; i < count ; i++) {
		...
		ret = CTP_SPI_READ(ts->client, data, len+1);
		if (ret) {
			NVT_ERR("Read fw checksum failed\n");
			return ret;
		}
...
return:
	n/a.
*******************************************************/
void nvt_update_gesture_firmware(char *firmware_name)
{
	...
	while (1) {
		...
		ret = nvt_write_gesture_firmware(fw_entry->data, fw_entry->size,
				SRAM_addr, size, BIN_addr, crc);
		if (ret) {
	...
	nvt_write_addr((ts->mmap->EVENT_BUF_ADDR | 0x53), 0x00);
	do_gettimeofday(&end);
...
#endif //#if NT36525_HDL_0_Flash_Gesture
