...
unsigned char FT5X46_TestItem_WeakShortTest( bool *bTestResult )
{
    ...
    for (i = 0; i < iChannelNum; i++) {
        ...
        if ((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0)) { //<=0 ShortResistance = 0
            iCount++;
            if (i + 1 <= iMaxTx)
                FTS_TEST_SAVE_INFO("\nTx%02d: %02d ", i + 1, fGShortResistance[i]);
            else
                FTS_TEST_SAVE_INFO("\nRx%02d: %02d ", i + 1 - iMaxTx, fGShortResistance[i]);
            if (iCount % 10 == 0)
                FTS_TEST_SAVE_INFO("\n\n");
        }
    ...
    for ( i = 0; i < iChannelNum; i++) {
        ...
        if ( fMShortResistance[i] <= 0  || fMShortResistance[i] < iMin_CC ) {
            iCount++;
            if (i + 1 <= iMaxTx)
                FTS_TEST_SAVE_INFO("\nTx%02d: %02d", i + 1, fMShortResistance[i]);
            else
                FTS_TEST_SAVE_INFO("\nRx%02d: %02d", i + 1 - iMaxTx, fMShortResistance[i]);
