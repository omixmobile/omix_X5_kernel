...
//Antaiui <AI_BSP_LCD> <wutj> <2022-09-15> add mmi lcm_device message display start
#include <linux/ai_device_check.h>
//Antaiui <AI_BSP_LCD> <wutj> <2022-09-15> add mmi lcm_device message display end


//Antai <AI_BSP_LCM> <chenht> <2022-12-08> add ai_st7785m_1lane_new lcm for 2106 begin
#include <linux/iio/consumer.h> 
static struct iio_channel *touch_ch3;
//Antai <AI_BSP_LCM> <chenht> <2022-12-08> add ai_st7785m_1lane_new lcm for 2106 end
...
int fts_register_hardware_info(void)
{
    struct ai_device_info ai_tp_hw_info;
	u8 fw_ver = 0;
	int ret = 0;
    ret = fts_fwupg_get_ver_in_tp(&fw_ver);
	ai_tp_hw_info.ai_dev_type = AI_DEVICE_TYPE_TP;
	snprintf(ai_tp_hw_info.name, AI_DEVICE_NAME_LEN, "fts_ts_fw[0x%x]",fw_ver);
...
int fts_get_profile_id(struct fts_upgrade *upg)
{
	int ret = -1;
    ...
    if (!touch_ch3){
        touch_ch3 = devm_kzalloc(upg->ts_data->dev, sizeof(*touch_ch3),GFP_KERNEL);
        if (!touch_ch3){
            FTS_ERROR("[%s]devm_kzalloc fail\n", __func__);
            return ret;
        }

        touch_ch3 = iio_channel_get(upg->ts_data->dev, "touch-id");
        touch_is_err = IS_ERR(touch_ch3);
        if (touch_is_err) {
            FTS_ERROR("[%s]iio_channel_get fail\n", __func__);
        }
    }
     

	ret = iio_read_channel_processed(touch_ch3, &auxadc_voltage);
	if (ret < 0) {
		FTS_ERROR("[%s] iio_read_channel_processed fail\n", __func__);
		return ret;
	}
    
    FTS_INFO("[%s]auxadc_voltage is %d\n", __func__, auxadc_voltage);
	
    

    if(auxadc_voltage < 1000){
		return AI_ST7785M;
	}else{
		return AI_ST7785M_NEW;
	}
...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg)
{
    ...
    if (fwvalid) {
        ...
        //Antai <AI_BSP_LCM> <chenht> <2022-12-08> add ai_st7785m_1lane_new lcm for 2106 begin
        lcm_id = fts_get_profile_id(upg);

        if (lcm_id == AI_ST7785M){
            if (fw_ver_in_tp < fw_ver_in_host && fw_ver_in_tp != 4) 
                return true;
        } else if (lcm_id == AI_ST7785M_NEW){
            if (fw_ver_in_tp < fw_ver_in_host)
                return true;
        } else {
            FTS_ERROR("lcm_id invalid, don't upgrade fw");
        }
        //Antai <AI_BSP_LCM> <chenht> <2022-12-08> add ai_st7785m_1lane_new lcm for 2106 end
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
    ...
    if (FTS_GET_MODULE_NUM > 1) {
        ...
        lcm_id = fts_get_profile_id(upg);
        if (lcm_id < 0) {
            FTS_ERROR("get_profile_id failed");
            return lcm_id;
        }
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
	...
	//Antaiui <AI_BSP_LCD> <chenht> <2022-12-8> modify mmi lcm_device message display start
    //#ifdef CONFIG_AI_BSP_MTK_DEVICE_CHECK
    //fts_register_hardware_info();
    //#endif
	//Antaiui <AI_BSP_LCD> <chenht> <2022-12-8> modify mmi lcm_device message display end
    return 0;
}
