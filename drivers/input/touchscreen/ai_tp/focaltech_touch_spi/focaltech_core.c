...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "fts_ts_SPI"
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ret = ai_fts_tpd_feature_init_data(ts_data);
	if(ret<0) {
		FTS_ERROR("[create focaltech feature node failed.\n");
	}
    //Antai <AI_BSP_TP> <chenht> <2022-11-30> modify for 2206 gesture end

    tpd_load_status = 1;
    FTS_INFO("tpd_load_status = 1---------------------");
    FTS_FUNC_EXIT();
    return 0;
...
//Antai <AI_BSP_TP> <chenht> <2022-11-30> add gesture for 2206 begin
#if FTS_GESTURE_EN
	ai_fts_tpd_feature_reinit();
#endif
//Antai <AI_BSP_TP> <chenht> <2022-11-30> add gesture for 2206  end	
...
//Antai <AI_BSP_TP> <chenht> <2022-11-28> disable avdd and avee for 2206 begin
extern int PMU_db_pos_neg_disable_delay(int ms);
//Antai <AI_BSP_TP> <chenht> <2022-11-28> disable avdd and avee for 2206 end
//Antai <AI_BSP_TP> <chenht> <2022-11-28> disable avdd and avee for 2206 begin
int is_gesture_mode_fts(void) {
	int ret = 0;
	//struct fts_ts_data *ts_data = fts_data;
    
#if FTS_GESTURE_EN
	if(fts_wake_switch || fts_gesture_switch)
		ret = 1;
	else
		ret = 0;
#else
...
EXPORT_SYMBOL(is_gesture_mode_fts);
//Antai <AI_BSP_TP> <chenht> <2022-11-28> disable avdd and avee for 2206 end
...
//Antai <AI_BSP_TP> <chenht> <2023-01-09> disable avdd and avee for 2206 begin	
#if FTS_GESTURE_EN
    if (fts_wake_switch || fts_gesture_switch) {
...
#endif
//Antai <AI_BSP_TP> <chenht> <2023-01-09> disable avdd and avee for 2206 end
    else {
