...
//Antaiui <AI_BSP_TP> <chenht> <2022-10-9> TP compatiable begin

#include <linux/iio/consumer.h> 
static struct iio_channel *battery_ch2;
//Antaiui <AI_BSP_TP> <chenht> <2022-10-9> TP compatiable end
...
int ovt_get_profile_id(void)
{
//Antaiui <AI_BSP_TP> <chenht> <2023-02-03> modified TP compatiable begin
	...
	if (ret < 0) {
		pr_err("[%s] iio_read_channel_processed fail\n", __func__);
		return ret;
	}
    
    pr_info("[%s]auxadc_voltage is %d\n", __func__, auxadc_voltage);
	
    if(auxadc_voltage <= 500){
		return 0;//omnivision_tcm
	}else if(auxadc_voltage > 500 && auxadc_voltage <= 1300){
		return 1;//chipone-tddi
	}else{
		return 2;//fts_ts_SPI
	}
		
	
//Antaiui <AI_BSP_TP> <chenht> <2023-02-03> modified TP compatiable end
}
...
//Antai <AI_BSP_LCD> <hehl> <2020-12-30> modify for 1919 end
	...
	//Antaiui <AI_BSP_TP> <chenht> <2022-10-9> TP compatiable begin
		...
		if (ret) {
			pr_err("[%s]iio_channel_get fail\n", __func__);
		}

	//Antaiui <AI_BSP_TP> <chenht> <2022-10-9> TP compatiable end

	//Antaiui <AI_BSP_TP> <chenht> <2022-10-9> TP compatiable begin
	lcd_id = ovt_get_profile_id();
	//Antaiui <AI_BSP_TP> <chenht> <2022-10-9> TP compatiable end
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			//Antaiui <AI_BSP_TP> <chenht> <2023-02-03> modified TP compatiable begin
			if(lcd_id == 0 && strcmp(tpd_driver_list[i].tpd_device_name, "omnivision_tcm") != 0) 
				continue;

			if(lcd_id == 1 && strcmp(tpd_driver_list[i].tpd_device_name, "chipone-tddi") != 0) 
				continue;

			if(lcd_id == 2 && strcmp(tpd_driver_list[i].tpd_device_name, "fts_ts_SPI") != 0) 
				continue;
			//Antaiui <AI_BSP_TP> <chenht> <2023-02-03> modified TP compatiable end
			tpd_driver_list[i].tpd_local_init();
			/* msleep(1); */
