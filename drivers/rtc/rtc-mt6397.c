...
enum mtk_rtc_spare_enum {
	...
	SPARE_SPAR0,
	SPARE_KPOC,
	//Antai <AI_BSP_FACTORY> <dantw> <2022-05-09> add for *#*#1923 enter factory mode begin
	#ifdef CONFIG_AI_BSP_CMD_TO_FACTORY_BOOTMENU
	RTC_BOOT_MENU,
	RTC_FACTORY,
	#endif
	//Antai <AI_BSP_FACTORY> <dantw> <2022-05-09> add for *#*#1923 enter factory mode end
	SPARE_RG_MAX,
...
static const struct reg_field mtk_rtc_spare_reg_fields[SPARE_RG_MAX] = {
	...
	//Antai <AI_BSP_FACTORY> <dantw> <2022-05-09> add for *#*#1923 enter factory mode begin
	#ifdef CONFIG_AI_BSP_CMD_TO_FACTORY_BOOTMENU
	[RTC_BOOT_MENU]		= REG_FIELD(RTC_PDN1, 10, 10),
	[RTC_FACTORY]		= REG_FIELD(RTC_PDN1, 11, 11),
	#endif
	//Antai <AI_BSP_FACTORY> <dantw> <2022-05-09> add for *#*#1923 enter factory mode end
};
...
//Antai <AI_BSP_FACTORY> <dantw> <2022-05-09> add for *#*#1923 enter factory mode begin
#ifdef CONFIG_AI_BSP_CMD_TO_FACTORY_BOOTMENU
struct mt6397_rtc *rtc1;
void rtc_mark_bootmenu(void)
{
    pr_notice("%s\n", __func__);
	//printk("dantangwei_rtc_mark_bootmenu1\n");
	mutex_lock(&rtc1->lock);
	regmap_field_write(rtc1->spare[RTC_BOOT_MENU], 1);
	mtk_rtc_write_trigger(rtc1);
	mutex_unlock(&rtc1->lock);
...
void rtc_mark_factory(void)
{
  	...
  	pr_notice("%s\n", __func__);
	//printk("dantangwei_rtc_mark_factory1\n");
	mutex_lock(&rtc1->lock);
	regmap_field_write(rtc1->spare[RTC_FACTORY], 1);
	mtk_rtc_write_trigger(rtc1);
	mutex_unlock(&rtc1->lock);
}
#endif
//Antai <AI_BSP_FACTORY> <dantw> <2022-05-09> add for *#*#1923 enter factory modeend
