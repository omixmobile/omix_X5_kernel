...
static struct charger_manager *charger_switch_info = NULL;
bool ui_soc_limit_charger_flag = false;
unsigned long current_limit_ui_soc = 101;
#ifdef CONFIG_AI_BSP_CHARGER_IC_HL7015A
extern void hl7019d_set_en_hiz(unsigned int val);
#endif
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-05-11> add reset ta hiz begin
extern void hl7019d_set_chg_config(unsigned int val);
//Antaiui <AI_BSP_CHG> <hehl> <2022-05-11> add reset ta hiz end
...
//Antaiui <AI_BSP_USB> <hehl> <2021-03-16> add uisoc limit charge interface #37743 begin               
        if((battery_get_uisoc() > current_limit_ui_soc || battery_get_uisoc() == current_limit_ui_soc )  && ui_soc_limit_charger_flag == true)
        {
                #ifdef CONFIG_AI_BSP_CHARGER_IC_HL7015A
                        hl7019d_set_chg_config(0);						
                #else   
                        charger_dev_enable_powerpath(info->chg1_dev, false);
                #endif

                chr_info("%s ui_soc limit , disable power path", __func__);
        }else if(battery_get_uisoc() < current_limit_ui_soc && ui_soc_limit_charger_flag == true){

                #ifdef CONFIG_AI_BSP_CHARGER_IC_HL7015A
                        hl7019d_set_chg_config(1);	
                #else   
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-20> ata charger begin
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-20> ata charger begin		
	...
	if (ret)
		goto _out;
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-20> ata charger end	
...
static ssize_t ai_charger_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        ...
        ui_soc_limit_charger_flag = false; 
                ...
                hl7019d_set_chg_config(1);
        #else
                charger_dev_enable_powerpath(charger_switch_info->chg1_dev, true);
        #endif
        current_limit_ui_soc = 101;
        return sprintf(buf, "%d", battery_get_uisoc());
}
...
//Antai <AI_BSP_CHG> <hehl> <2022-04-14> add fast charger switch for TK begin
...
#endif
//Antai <AI_BSP_CHG> <hehl> <2022-04-14> add fast charger switch for TK end
