...
extern unsigned long fast_charger_en;
#endif
...
//Antai <AI_BSP_CHG> <hehl> <2022-04-14> add fast charger switch for TK begin
#ifdef CONFIG_AI_BSP_FAST_CHARGER_SWITCH
	if ((pinfo->data.ta_stop_battery_soc < battery_get_soc() && ichg > 0 && ichg < pinfo->data.pe_ichg_level_threshold) || (fast_charger_en == 0)) {
#else	
...
//Antai <AI_BSP_CHG> <hehl> <2022-04-14> add fast charger switch for TK begin
#ifdef CONFIG_AI_BSP_FAST_CHARGER_SWITCH
    if(fast_charger_en == 0)
    {
        chr_info("%s: close fast charger\n", __func__);
        return ret;
    }
#endif
//Antai <AI_BSP_CHG> <hehl> <2022-04-14> add fast charger switch for TK end
