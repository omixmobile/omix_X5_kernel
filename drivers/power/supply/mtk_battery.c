...
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-22> modify mmi charger infomation begin
#ifdef CONFIG_AI_BSP_MMITEST_CHGCUR
int g_ntc_status = 1;
#endif
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-22> modify mmi charger infomation end
//Antaiui <AI_BSP_CHG> <zhul> <2022-10-09> battery compatiable begin
#if defined(CONFIG_AI_BSP_MULTI_BAT_SUPPORT) 
#include <linux/iio/consumer.h> 
struct iio_channel *battery_ch2;
#endif
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
#include <linux/ai_device_check.h>
//extern int ai_set_device_info(struct ai_device_info ai_dev_info);
#endif
//Antaiui <AI_BSP_CHG> <zhul> <2022-10-09> battery compatiable end
...
//Antaiui <AI_BSP_CHG> <zhul> <2022-10-09> battery compatiable begin
    ...
    int battery_id = TOTAL_BATTERY_NUMBER - 1;
	ret = iio_read_channel_processed(battery_ch2, &auxadc_voltage);
	if (ret < 0) {
		bm_err("[%s] iio_read_channel_processed fail\n", __func__);
		return ret;
	}
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if (auxadc_voltage < g_battery_id_voltage[id]) {
			...
			battery_id = TOTAL_BATTERY_NUMBER - 1;
		}
	}
...
//Antaiui <AI_BSP_Bat> <yaoyc> <2021-05-25> modify for li-ion polymer battery begin
	bs_data->bat_technology = POWER_SUPPLY_TECHNOLOGY_LIPO,
//Antaiui <AI_BSP_Bat> <yaoyc> <2021-05-25> modify for li-ion polymer battery end
...
//Antaiui <AI_BSP_CHG> <penggy> <2021-07-23> add disable GM 3.0 define begin	
#if defined(CONFIG_AI_BSP_DISABLE_GM30)
	gm->fixed_bat_tmp = 25;
#endif
//Antaiui <AI_BSP_CHG> <penggy> <2021-07-23> add disable GM 3.0 define end	
...
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
{
	struct ai_device_info ai_mydev_info_bat;
	ai_mydev_info_bat.ai_dev_type = AI_DEVICE_TYPE_BATTERY;

	#if defined(CONFIG_AI_BSP_MULTI_BAT_SUPPORT) 
		strcpy(ai_mydev_info_bat.name, g_battery_id_information[gm->battery_id]);
	#else
		strcpy(ai_mydev_info_bat.name, "No battery information");
	#endif
	ai_set_device_info(ai_mydev_info_bat);
}
#endif
//Antaiui <AI_BSP_CHG> <zhul> <2022-10-09> battery compatiable end
		}
...
{
//Antaiui <AI_BSP_CHG> <hehl> <2021-07-15> add disable GM 3.0 define begin	
#if defined(CONFIG_AI_BSP_DISABLE_GM30)
	if(1){
		printk("disable GM 3.0\n");
#else		
	if (gm->disableGM30) {
#endif
//Antaiui <AI_BSP_CHG> <hehl> <2021-07-15> add disable GM 3.0 define end			
		disable_fg(gm);
		return false;
	}
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf end
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf end                
	...
	if (sdd->shutdown_status.is_uisoc_one_percent) {
		...
		if (current_ui_soc == 0) {
			...
			if (duraction.tv_sec >= SHUTDOWN_TIME) {
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin                
				printk("uisoc one percent shutdown\n");
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf end                
				kernel_power_off();
				return next_waketime(polling);
			}
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin            
			printk("disable uisoc_one_percent shutdown cur:%d soc:%d\n",
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin
		printk("lbatcheck vbat:%d avgvbat:%d %d,%d tmp:%d,bound:%d,th:%d %d,en:%d\n",
			vbat,
			sdd->avgvbat,
			sdd->vbat_lt,
			sdd->vbat_lt_lv1,
			...
			if (down_to_low_bat == 0) {
				if (IS_ENABLED(
					...
					if (tmp >= LOW_TEMP_THRESHOLD) {
						down_to_low_bat = 1;
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin                        
						printk("normal tmp, battery voltage is low shutdown\n");
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf end                        
						wakeup_fg_algo(gm,
							FG_INTR_SHUTDOWN);
					} else if (sdd->avgvbat <=
						...
						down_to_low_bat = 1;
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin                        
						printk("cold tmp, battery voltage is low shutdown\n");
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf end                        
						wakeup_fg_algo(gm,
							FG_INTR_SHUTDOWN);
					} else
					...
					down_to_low_bat = 1;
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin                    
					printk("[%s]avg vbat is low to shutdown\n",
						__func__);
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf end
					wakeup_fg_algo(gm, FG_INTR_SHUTDOWN);
				}
		...
		polling++;
//Antaiui <AI_BSP_CHG> <hehl> <2022-04-14> modify printf begin        
			printk("[%s][UT] V %d ui_soc %d dur %d [%d:%d:%d:%d] batdata[%d] %d\n",
				__func__,
...
//Antai <AI_BSP_CHG> <hehl> <2022-02-15> modify begin                
			if (gm->cur_bat_temp >= BATTERY_SHUTDOWN_TEMPERATURE || gm->cur_bat_temp <= -10) {
//Antai <AI_BSP_CHG> <hehl> <2022-02-15> modify end                
				bm_debug(
...
//Antaiui <AI_BSP_CHG> <zhul> <2022-10-09> battery compatiable begin
#if defined(CONFIG_AI_BSP_MULTI_BAT_SUPPORT) 
	battery_ch2 = devm_kzalloc(&pdev->dev, sizeof(*battery_ch2),GFP_KERNEL);
	if (!battery_ch2)
		bm_err("[%s]devm_kzalloc fail\n", __func__);


	battery_ch2 = iio_channel_get(&pdev->dev, "battery-id");
	ret = IS_ERR(battery_ch2);
	if (ret) {
		bm_err("[%s]iio_channel_get fail\n", __func__);
	}
#endif	
//Antaiui <AI_BSP_CHG> <zhul> <2022-10-09> battery compatiable end
