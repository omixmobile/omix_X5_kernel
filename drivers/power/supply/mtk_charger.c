...
struct mtk_charger *charger_switch_info = NULL;
bool ui_soc_limit_charger_flag = false;
unsigned long current_limit_ui_soc = 101;
...
//Antai <AI_BSP_CHG> <hehl> <2022-02-17> modify #38171 for 2008IK begin
#ifdef CONFIG_AI_BSP_ASW2008IK_CHARGER_VOLTAGE_LIMIT
    if (of_property_read_u32(np, "recover_charger_voltage", &val) >= 0)
		info->data.recover_charger_voltage = val;
	else {
		chr_err("use default V_CHARGER_MAX:%d\n", V_CHARGER_MAX);
		info->data.recover_charger_voltage = V_CHARGER_MAX;
	}
#endif
//Antai <AI_BSP_CHG> <hehl> <2022-02-17> modify #38171 for 2008IK end
...
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-23> ata charger begin
static ssize_t Ata_Get_Input_Current_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	int ibus = get_ibus(pinfo); /* mV */
    ibus = 480;
	printk("%s: %d\n", __func__, ibus);
	return sprintf(buf, "%d\n", ibus);
}

static DEVICE_ATTR_RO(Ata_Get_Input_Current);
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-23> ata charger end
...
//Antai <AI_BSP_CHG> <hehl> <2022-02-17> modify #38171 for 2008IK begin
#ifdef CONFIG_AI_BSP_ASW2008IK_CHARGER_VOLTAGE_LIMIT
enum charger_voltage_state_enum {
  	CHARGER_VOLTAGE_NORMAL = 0,
...
//Antai <AI_BSP_CHG> <hehl> <2022-02-17> modify #38171 for 2008IK begin	
	...
	if (vchr > info->data.max_charger_voltage) {
        info->charger_voltage_state = CHARGER_VOLTAGE_HIGH;
		chr_err("%s: vbus(%d mV) > %d mV\n", __func__, vchr / 1000,
			info->data.max_charger_voltage / 1000);
		return false;
	}else if(info->charger_voltage_state == CHARGER_VOLTAGE_HIGH){
        if(vchr < info->data.recover_charger_voltage)
        {
            info->charger_voltage_state = CHARGER_VOLTAGE_NORMAL;
            chr_err("%s: vbus(%d mV) drop to %d mV , recover charger\n", __func__, vchr / 1000,
                info->data.recover_charger_voltage / 1000);  
            return true;
        }else{
            return false;
        }
...
//Antaiui <AI_BSP_CHG> <hehl> <2022-06-14> low temp Pop up prompt begin
		if (info->battery_temp <= info->thermal.min_charge_temp) {
//Antaiui <AI_BSP_CHG> <hehl> <2022-06-14> low temp Pop up prompt end            
			info->notify_code |= CHG_BAT_LT_STATUS;
			chr_err("bat_temp(%d) out of range(too low)\n",
				info->battery_temp);
			mtk_chgstat_notify(info);
...
//Antaiui <AI_BSP_USB> <hehl> <2021-03-16> add uisoc limit charge interface begin			
	if((get_uisoc(info) > current_limit_ui_soc || get_uisoc(info) == current_limit_ui_soc )  && ui_soc_limit_charger_flag == true)
	{	  
            charger_dev_enable_powerpath(info->chg1_dev, 0);
		chr_info("%s ui_soc limit , disable charge", __func__);
	}else if(get_uisoc(info) < current_limit_ui_soc && ui_soc_limit_charger_flag == true){  
            charger_dev_enable_powerpath(info->chg1_dev, 1);
		chr_info("%s lower ui_soc limit , recover charge", __func__);		
	}
//Antaiui <AI_BSP_USB> <hehl> <2021-03-16> add uisoc limit charge interface end		
}
...
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-23> ata charger begin		
	ret = device_create_file(&(pdev->dev), &dev_attr_Ata_Get_Input_Current);
	if (ret)
		goto _out;
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-23> ata charger end
...
//Antaiui <AI_BSP_USB> <hehl> <2021-03-16> add uisoc limit charge interface begin
static ssize_t ai_charger_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("%s\n", __func__);
	ui_soc_limit_charger_flag = false;
	charger_dev_enable_powerpath(charger_switch_info->chg1_dev, 1);
	return sprintf(buf, "%d", get_uisoc(charger_switch_info));
}
static ssize_t ai_charger_switch_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	
	ret = kstrtoul(buf, 10, &current_limit_ui_soc); 
	if(current_limit_ui_soc < 0 || current_limit_ui_soc > 100 || ret != 0)
	{
		chr_info("%s invalid value", __func__);
	}else {	
		ui_soc_limit_charger_flag = true;
		chr_info("%s store limit_soc=%d", __func__,current_limit_ui_soc);
	}
	return count;
}
static DEVICE_ATTR (charger_switch, S_IRUGO|S_IWUSR, ai_charger_switch_show, ai_charger_switch_store);
//Antai <AI_BSP_CHG> <yaoyc> <2022-11-24> add fast charger switch for TK begin
#ifdef CONFIG_AI_BSP_FAST_CHARGER_SWITCH
unsigned long fast_charger_en = 1;
static ssize_t ai_fast_charger_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
        printk("%s\n", __func__);

        return sprintf(buf, "%d", fast_charger_en);
}
static ssize_t ai_fast_charger_switch_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	
	ret = kstrtoul(buf, 10, &fast_charger_en); 

	chr_info("%s store fast_charger_en=%d", __func__,fast_charger_en);

	return count;
}
static DEVICE_ATTR (fast_charger_switch, S_IRUGO|S_IWUSR, ai_fast_charger_switch_show, ai_fast_charger_switch_store);
#endif
//Antai <AI_BSP_CHG> <yaoyc> <2022-11-24> add fast charger switch for TK end
static struct device_attribute *charger_switch_attr_list[] = {
    &dev_attr_charger_switch,
//Antai <AI_BSP_CHG> <yaoyc> <2022-11-24> add fast charger switch for TK begin 
#ifdef CONFIG_AI_BSP_FAST_CHARGER_SWITCH  
    &dev_attr_fast_charger_switch,
#endif
//Antai <AI_BSP_CHG> <yaoyc> <2022-11-24> add fast charger switch for TK end
};
static struct platform_device ai_charger_switch_device = {
    .name   = "charger_switch_state",
    .id     = -1,
};
static int charger_switch_create_attr(struct device *dev)
{
    int idx, err = 0;
    int num = (int)(sizeof(charger_switch_attr_list)/sizeof(charger_switch_attr_list[0]));
    ...
    {
        if(err != device_create_file(dev, charger_switch_attr_list[idx]))
        {
			chr_err("%s: driver_create_file failed\n", __func__);
            break;
        }
    }
...
//Antaiui <AI_BSP_USB> <hehl> <2021-03-16> add uisoc limit charge interface begin
	charger_switch_info = info;
	ret = platform_device_register(&ai_charger_switch_device);
    if (ret)
    {
        chr_err("create ai_charger_switch_device failed\n");
    }
    ret = charger_switch_create_attr(&(ai_charger_switch_device.dev));
    if(ret)
    {
        chr_err("create ai_charger_switch_device attr failed \n");
    }
//Antaiui <AI_BSP_USB> <hehl> <2021-03-16> add uisoc limit charge interface end	
	return 0;
}
