...
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
	char nxp_id[3][20]={"NXP-PN553","null","null"};
	struct ai_device_info ai_nfc_hw_info;
#endif
    pr_debug("%s: enter\n", __func__);
...
#if defined(CONFIG_AI_BSP_MTK_DEVICE_CHECK)
	ai_nfc_hw_info.ai_dev_type = AI_DEVICE_TYPE_NFC;
	
	snprintf(ai_nfc_hw_info.name, AI_DEVICE_NAME_LEN, "%s",nxp_id[0]);
		
	ai_set_device_info(ai_nfc_hw_info);
#endif
    return 0;
